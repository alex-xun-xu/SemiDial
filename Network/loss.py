...
def distribution10(vector):
    distribution = [0] * 10

    for value in vector:
        interval_index = int(value * 10)
        if (interval_index == 10.0):
            distribution[-1] += 1
        else:
            distribution[interval_index] += 1

    sum_distribution = sum(distribution)
    for index in range(10):
        distribution[index] = (distribution[index]/sum_distribution)*100
    output = ' '.join([f"{value:.2f}%  " for value in distribution])
    print(output)
...
class DiceLoss(nn.Module):
    def __init__(self, logit=True, smooth=1e-1,logit2=False):
        super().__init__()
        self.logit = logit
        self.smooth = smooth
        self.logit2 = logit2
...
class DiceLoss2(nn.Module):
    def __init__(self, logit=True, smooth=1e-10):
        super().__init__()
        self.logit = logit
        self.smooth = smooth
...
class Dice_ConsistMSELoss(nn.Module):
    ...
    def forward(self, **kwargs):
        pred_labeled = kwargs['pred_labeled']
        gt_labeled = kwargs['gt_labeled']

        if 'pred_unlabeled' in kwargs:
            pred_unlabeled = kwargs['pred_unlabeled']
        else:
            pred_unlabeled = None
        ...
        loss_mse_unlabeled = 0.
        if pred_unlabeled is not None:
            loss_mse_unlabeled = self.loss_mse(pred_unlabeled[0], pred_unlabeled[1])

        return loss_dice_labeled + weight * (loss_mse_labeled + loss_mse_unlabeled), loss_dice_labeled, loss_mse_labeled, loss_mse_unlabeled



from collections import Counter
def distribution10(vector):
    interval_indices = (vector * 10).floor().clamp(0, 9).int()
    counter = Counter(interval_indices.numpy())
    total_length = len(vector)
    probabilities = []
    for i in range(10):
        count = counter.get(i, 0)
        probability = count / total_length * 100.0
...
class Dice_MaskedConsistMSELoss(nn.Module):
    def __init__(self, logit, taus=[],args=None,loader=None):
        super().__init__()
        self.dice_loss = DiceLoss(True)
        self.dice_lossUnLogit = DiceLoss(False)
        self.dice_lossGtLogitPredLogit = DiceLoss(logit=True, logit2=True)
        self.loss_mseLogit = MSELoss(True)
        self.loss_mseUnLogit = MSELoss(False)
        self.args = args
        self.loader = loader
        self.baseline = args.BaseLine
        self.queue_size = math.ceil(self.loader.num_train * (1 - self.loader.label_percent) / (0.5 * args.batchsize))
        self.pseudo_queue = queue.Queue(maxsize=self.queue_size)

        if self.baseline == 'BaseLine3-5':
            self.best_tau_iter = {
                "best_tau_iter":[]
            }
    ...
    def calculate_iou(self, tensor1, tensor2):
    ...
    def calculate_kld(sele, P, Q):
        kld = 0.0
        for i in range(len(P)):
            if float(P[i]) == 0.0:
                P[i] = 1e-20
            kld += P[i] * math.log10(float(P[i]) / float(Q[i]))
        return kld
    ...
    def forward(self, **kwargs):
        pred_labeled = kwargs['pred_labeled']
        gt_labeled = kwargs['gt_labeled']
        gt_unlabeled = kwargs['gt_unlabeled']
        label_distribution = kwargs['label_distribution']
        ...
        unlabel_pred = pred_unlabeled[0]
        unlabel_pseudo = pred_unlabeled[1]

        if pred_unlabeled is not None:
            if self.baseline == 'None':
                loss_mse_unlabeled = self.loss_mseLogit(input1=unlabel_pred, input2=unlabel_pseudo)

            elif self.baseline == 'None2':
                loss_mse_unlabeled = self.dice_lossGtLogitPredLogit(pred=unlabel_pred, gt=unlabel_pseudo)

            elif self.baseline == "BaseLine3-5":
                tau3_5s = [0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]
                best_tau3_5 = 0.1
                best_kld = 9999999999
                klds = []

                unlabel_pred = torch.sigmoid(unlabel_pred)
                unlabel_pseudo = torch.sigmoid(unlabel_pseudo)

                for tau3_5 in tau3_5s:
                    unlabel_pseudo_temp = unlabel_pseudo.clone()
                    unlabel_pseudo_temp[unlabel_pseudo >= tau3_5] = 1
                    unlabel_pseudo_temp[unlabel_pseudo < tau3_5] = 0

                    foreground_pseudo_distribution = unlabel_pseudo_temp.mean()
                    background_pseudo_distribution = 1 - foreground_pseudo_distribution
                    pseudo_distribution = [foreground_pseudo_distribution, background_pseudo_distribution]

                    kld = self.calculate_kld(pseudo_distribution, label_distribution).item()
                    klds.append(kld)
                    print("tau3_5 = {:.4f}   foreground_pseudo = {:.4f} %   KLD = {:.9f}".format(
                        tau3_5,foreground_pseudo_distribution.item() * 100.0,kld))
                    if kld <= best_kld:
                        best_kld = kld
                        best_tau3_5 = tau3_5

                self.best_tau_iter["best_tau_iter"].append(best_tau3_5)

                unlabel_pseudo[unlabel_pseudo >= best_tau3_5] = 1
                unlabel_pseudo[unlabel_pseudo < best_tau3_5] = 0

                loss_mse_unlabeled = self.dice_lossUnLogit(pred=unlabel_pred, gt=unlabel_pseudo)

            else:
                print("【loss/Dice_MaskedConsistMSELoss】error")

            print("【loss/Dice_MaskedConsistMSELoss】cur mini-batch  labeled_loss = {:.4f}  unlabeled_loss = {:.4f}  weight = {:.4f}  loss = {:.4f}".format(loss_dice_labeled, loss_mse_unlabeled, weight, loss_dice_labeled + weight * loss_mse_unlabeled))
