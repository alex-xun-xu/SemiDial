...
class evaluate():
    ...
    def persamp_dice(self, pred, gt):
        interesect = np.sum(pred.astype(float) * gt.astype(float), axis=(1, 2))
        union = np.sum(pred.astype(float) + gt.astype(float), axis=(1, 2))
        persamp_dice = 2 * interesect / (union + 1e-10)

        return persamp_dice


    def micro_dice(self, pred, gt):
        pred = pred.reshape(-1)
        gt = gt.reshape(-1)

        interesect = np.sum(pred.astype(float) * gt.astype(float))
        union = np.sum(pred.astype(float) + gt.astype(float))
        micro_dice = 2 * interesect / (union + 1e-10)
