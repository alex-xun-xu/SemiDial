...
class ResUNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True, ki='glorot_uniform',seed_model=0):
        super().__init__()
    ...
    def forward(self, x):
        ## Encoder Net
        out = self.ResBlock1(x.contiguous())
        res1 = out.clone()
...
sys.path.append(os.path.join(pathlib.Path(__file__).parent.absolute().parent.absolute(), 'pytorch-deeplab-xception'))
