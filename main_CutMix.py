...
parser.add_argument('--BaseLine',
                    type=str,
                    choices=['None', 'BaseLine1', 'BaseLine2', 'BaseLine3-1', 'BaseLine3-2', 'BaseLine3-3'],
                    default= 'None2')
parser.add_argument('--Taus',
                    type=str,
                    default= [0.95, 0.0, 0.0])
parser.add_argument('--Pretrain',
                    type=str,
                    choices=[True, False],
                    default=False)
parser.add_argument('--Tau0Bin',
                    type=str,
                    choices=[True, False],
                    default=False)
...
parser.add_argument('--loss','-l',
                    ...
                    default='Dice+MaskedConsistMSE_loss')
...
if 'Contrastive' not in args.loss:
    ...
    elif args.ssl == 'CutMix2':
        import trainer_unet_CutMix2 as trainer
    elif args.ssl == 'SEM':
...
if args.Mode == 'train':
    ...
    with torch.no_grad():
        ...
        if args.ExportFigure:
            loss_te, macro_dice_te, micro_dice_te, perpix_acc_te, macro_iou_te, micro_iou_te, AIU = Trainer.TestAll_SavePred(exp_fig=True)
        else:
            loss_te, macro_dice_te, micro_dice_te, perpix_acc_te, macro_iou_te, micro_iou_te, AIU = Trainer.TestAll_SavePred(exp_fig=False)
