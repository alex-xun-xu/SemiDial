...
class Trainer(UnetTrainer):
    ...
    def SaveEpochResult(self):
        ...
        if self.baseline == 'BaseLine3-5':
            path_best_tau_iter = os.path.join(self.result_path, 'best_tau_iter.csv')
            result_best_tau_iter = pd.DataFrame(data=self.criterion.best_tau_iter)
            result_best_tau_iter.to_csv(path_best_tau_iter, index=False)
        ...
        while True:
            ...
            if w != 0.0:
                ...
                for p_i in range(nUnlabel):
                    ...
                    p2_i = np.random.choice(np.delete(np.arange(0, nUnlabel), p_i), 1)[
                        0]  # randomly chosen xb from minibatch
                ...
                train_unlabeled_data_view1 = torch.tensor(train_unlabeled_data_view1, device=self.device,
                                                          dtype=torch.float32)
                train_unlabeled_data_viewstu = torch.tensor(train_unlabeled_data_viewstu, device=self.device,
                                                            dtype=torch.float32)
            ...
            train_labeled_pred_ema = self.net_ema(
                train_labeled_data_view1)  # forward pass prediction of train labeled set view2
            ...
            if w != 0.0:
                ...
                train_unlabeled_pred_viewstu = self.net(
                    train_unlabeled_data_viewstu)  # forward pass prediction of train unlabeled set view1
                train_unlabeled_pred_viewteach = self.net_ema(
                    train_unlabeled_data_view1)  # forward pass prediction of train unlabeled set view2
                #
                # blend mixed images
    ...
    def TestAll_TopoPostProc(self, exp_fig=False, best_ckpt_filepath=None):
        ...
        while True:
            ...
            for pred_i in test_pred:
                ...
                x_init = torch.tensor(pred_i[0].detach().cpu().numpy(), device=pred_i.device, requires_grad=True).type(
                    torch.float)
                x_t = torch.tensor(pred_i[0].detach().cpu().numpy(), device=pred_i.device, requires_grad=True).type(
                    torch.float)
                x_t = torch.autograd.Variable(
                    torch.tensor(pred_i[0].detach().cpu().numpy(), device=pred_i.device).type(torch.float),
                    requires_grad=True)
    ...
    def TestAll_RandCropTopoPostProc(self, exp_fig=False, best_ckpt_filepath=None):
        ...
        tloss = Loss.TopoLoss([int(self.TopoSettings['CropSize'] / 2 ** self.TopoSettings['DownsampTimes']),
                               int(self.TopoSettings['CropSize'] / 2 ** self.TopoSettings[
                                   'DownsampTimes'])])  # topology penalty
        gloss = nn.MSELoss()
        ...
        while True:
            ...
            for name_i, pred_i in zip(test_names, test_pred):
                ...
                for rep in range(self.TopoSettings['RandCrop']):
                    ...
                    # x_init = pred_i[0].detach()
                    x_init = torch.tensor(pred_i[0].detach().cpu().numpy(), device=pred_i.device,
                                          requires_grad=False).type(torch.float)
                    # crop prediction map
                    ...
                    y = int(y)
                    x_init_crop = x_init[
                                  x - int(self.TopoSettings['CropSize'] / 2):x + int(self.TopoSettings['CropSize'] / 2),
                                  y - int(self.TopoSettings['CropSize'] / 2):y + int(
                                      self.TopoSettings['CropSize'] / 2)]  # detach from graph
            ...
            for name_i, pred_i in zip(test_names, test_pred):
                ...
                for rep in range(self.TopoSettings['RandCrop']):
                    ...
                    for i in range(self.TopoSettings['OptIters']):
                        ...
                        tlossi, dgminfo = tloss(x_t)
                        glossi = gloss(x_t, x_init_crop)
                        loss = (self.TopoSettings['TopologyWeight'] * tlossi + self.TopoSettings[
                            'GeometricWeight'] * glossi) \
